...
class AdminPostController extends AdminbaseController {
    ...
    function addnotice_post(){
    	...
    	$article['post_content'] = I('post_content');
    	$article['post_content']=htmlspecialchars_decode($article['post_content']);
        
       $map = array('gnotice' => $article['post_content'],
       	             'gtitle' =>  $article['post_title']);
         $addn = M('gnotice')->add($map);
         if ($addn) {
         $this->success("添加成功！",U('AdminPost/addnotice'));
         } else {
         $this->error("添加失败！");
         }
